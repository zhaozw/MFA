...
public class testchoose extends Activity {
	...
	public void onClick(View v) {
		...
		case R.id.SendChristian: {
			...
			Log.d("sending hit to christian", setHit + "?From="
					+ db.getUserDetails().get("uid").toString().trim()
					+ "&To=5150e7b96b14f7.58739785&Type=" + hitchoice + "&Msg="
					+ s.trim());
			...
			if (hitavailiable(uid)) {
				try {
					...
					HttpPost httpPost = new HttpPost((setHit + "?From="
							+ db.getUserDetails().get("uid").toString().trim()
							+ "&To=5150e7b96b14f7.58739785&Type=" + hitchoice
							+ "&Msg=" + s));
		...
		case R.id.SendYacov: {
			...
			Log.d("sending hit to christian", setHit + "?From="
					+ db.getUserDetails().get("uid").toString().trim()
					+ "&To=513557a6a1c802.11086283&Type=" + hitchoice + "&Msg="
					+ s.trim());
			...
			if (hitavailiable(uid)) {
				try {
					...
					HttpPost httpPost = new HttpPost((setHit + "?From="
							+ db.getUserDetails().get("uid").toString().trim()
							+ "&To=513557a6a1c802.11086283&Type=" + hitchoice
							+ "&Msg=" + s));
		...
		case R.id.SendKAcie: {
			...
			Log.d("sending hit to christian", setHit + "?From="
					+ db.getUserDetails().get("uid").toString().trim()
					+ "&To=513be7b18d0511.81928076&Type=" + hitchoice
					+ "&Msg=fuck you");
		...
		case R.id.SendJames: {
			...
			Log.d("sending hit to christian", setHit + "?From="
					+ db.getUserDetails().get("uid").toString().trim()
					+ "&To=51354f4b171520.96868586&Type=" + hitchoice
					+ "&Msg=fuck you");
	...
	private boolean hitavailiable(String uid2) {
		...
		JSONObject json = jParser
				.getJSONFromUrl("http://cofactor1-unbrandedtech.dotcloud.com/Get_Hits.php?hits="
						+ uid2);
