...
public class AIPack {
    ...
    public AI[] aiPack;
    public int topTotal,bottomTotal,topLast,bottomLast,front,patternTimer=500,position=0;
...
public void updateFollowers() {
    for(int k=0;k<aiPack.length;k++){
        if(aiPack[k].following==aiPack[front]&&aiPack[k].following.dead==true){
             for(int j=0;j<aiPack.length;j++){
                    if(aiPack[j].following==aiPack[front]&&aiPack[j].following.dead==true&&j!=k)
