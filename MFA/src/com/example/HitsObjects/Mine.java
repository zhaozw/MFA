...
public class Mine {
	...
	public Mine(Bitmap img){
        ...
        this.x= generator.nextInt(MGP.deviceWidth*3)+MGP.deviceWidth+(int) MGP.dp[150]; 
	...
	public void exploded(){
		 ...
		 activated=false;
		 activationTime=100;
		 this.x= generator.nextInt(MGP.deviceWidth)+MGP.deviceWidth+(int) MGP.dp[150];
		 cx=x + (width / 2);
	     cy=y + (height / 2);	
	}
	 ...
	 public boolean shipInnerCollision(Player ship)
	    { 
	        ...
	        if(Math.pow(innerRadius+ship.radius,2) > Math.pow(ship.cx-cx,2) + Math.pow(ship.cy-cy,2))
	          return true; 
	        return false; 
	    }
	 ...
	 public boolean shipOuterCollision(Player ship)
	    { 
	        //using the distance formula to check for collisions.
	        if(Math.pow(outerRadius+ship.radius,2) > Math.pow(ship.cx-cx,2) + Math.pow(ship.cy-cy,2))
	          return true; 
	        return false; 
	    }
	 ...
	 public boolean shotCollision(Shot shot)
	    { 
	        // Uses the distance formula to check for collisions with other shots.
	          return (Math.pow(innerRadius,2) > Math.pow(shot.x-cx,2)+ Math.pow(shot.y-cy,2));
	    }
	 
	 public boolean mineOuterCollision(Mine mine)
	    { 
	        //using the distance formula to check for collisions.
	        if(Math.pow(outerRadius+mine.outerRadius,2) > Math.pow(mine.cx-cx,2) + Math.pow(mine.cy-cy,2))
	          return true; 
	        return false; 
	    }
