...
public class AnalogStick 
{
	...
	public AnalogStick(int s,int as)
	{
	    ...
	    cx=(x+(size/2));
	    cy=(y+(size/2));
	    activeLocation = new RectF(cx-(analogSize/2),cy-(analogSize/2),cx+(analogSize/2),cy+(analogSize/2));
	...
	{
		activeLocation.set(x-(analogSize/2),y-(analogSize/2),x+(analogSize/2),y+(analogSize/2));
		acx=((int)activeLocation.left+(analogSize/2));
		acy=((int)activeLocation.top+(analogSize/2));
	...
	{
		activeLocation.set(cx-(analogSize/2),cy-(analogSize/2),cx+(analogSize/2),cy+(analogSize/2));
		acx=((int)activeLocation.left+(analogSize/2));
		acy=((int)activeLocation.top+(analogSize/2));
		xDiff=acx-cx;
	    yDiff=acy-cy;
